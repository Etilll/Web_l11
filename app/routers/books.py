...
async def all_contacts(db: Session=Depends(get_db), 
    ...
    response = await contact_funcs.get_contacts(db=db,id=id,name=name,surname=surname,mail=mail,phone=phone)
    if response == []:
...
@router.post("/")
async def add_contact(contact: PostContact, db: Session = Depends(get_db)):
    response = await contact_funcs.add_contact(contact=contact,db=db)
...
async def del_contact(contact_id: Annotated[int, Path(title="The id of a contact to delete")], db: Session = Depends(get_db)):
    response = await contact_funcs.delete_contact(contact_id=contact_id, db=db)
    if response:
        return response
    else:
...
async def put_contact(contact_id: Annotated[int, Path(title="The id of a contact to delete")], db: Session = Depends(get_db), 
                    id: Annotated[int | None, Query(alias="id", example="id=42")]=None,
                    name: Annotated[str | None, Query(alias="name", example="name=Jone")]=None, 
                    surname: Annotated[str | None, Query(alias="surname", example="surname=Jenkins")]=None, 
                    mail: Annotated[str | None, Query(alias="mail", example="mail=123@gmail.com")]=None, 
                    phone: Annotated[str | None, Query(alias="phone", example="phone=380731290102")]=None): 
    response = await contact_funcs.put_contact(contact_id=contact_id, id=id, name=name, surname=surname, mail=mail, phone=phone)
    if response:
        return response
    else:
        raise HTTPException(status_code=404, detail="No specified contact was found!")
...
async def calc_birthdays(db: Session = Depends(get_db)):
    response = await contact_funcs.calculate_birthdays(db=db)
    return response
